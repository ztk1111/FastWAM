...
def _evaluate(
    ...
    model.eval()
    totals = {"loss": 0.0, "recon_loss": 0.0, "kl_loss": 0.0, "jerk_loss": 0.0}
    count = 0
    with torch.no_grad():
...
def main():
    ...
    jerk_weight = float(cfg.train.get("jerk_weight", 0.0))
    jerk_dims = int(cfg.train.get("jerk_dims", 6))
        ...
        for batch in train_loader:
            ...
            if log_every > 0 and step % log_every == 0:
                ...
                print(
                    ...
                    f"loss={metrics['loss']:.6f} recon={metrics['recon_loss']:.6f} "
                    f"kl={metrics['kl_loss']:.6f} jerk={metrics['jerk_loss']:.6f} "
            ...
            if val_loader is not None and eval_every > 0 and step % eval_every == 0:
                ...
                print(
                    ...
                    f"jerk={val_metrics['jerk_loss']:.6f}"
                )
