...
def action_vae_loss(
    ...
    jerk_loss = target.new_zeros(())
    jerk_dim_count = min(int(jerk_dims), target.shape[-1])
    if float(jerk_weight) > 0.0 and jerk_dim_count > 0 and target.shape[1] >= 4:
        pred = output.reconstruction[..., :jerk_dim_count]
        target_for_jerk = target[..., :jerk_dim_count]
        pred_jerk = pred[:, 3:] - 3.0 * pred[:, 2:-1] + 3.0 * pred[:, 1:-2] - pred[:, :-3]
        target_jerk = (
            target_for_jerk[:, 3:]
            - 3.0 * target_for_jerk[:, 2:-1]
            + 3.0 * target_for_jerk[:, 1:-2]
            - target_for_jerk[:, :-3]
        )
        jerk_error = F.mse_loss(pred_jerk, target_jerk, reduction="none").mean(dim=-1)
        if action_is_pad is not None:
            jerk_valid = (
                (~action_is_pad[:, 3:].bool())
                & (~action_is_pad[:, 2:-1].bool())
                & (~action_is_pad[:, 1:-2].bool())
                & (~action_is_pad[:, :-3].bool())
            ).to(dtype=jerk_error.dtype)
            jerk_loss = (jerk_error * jerk_valid).sum() / jerk_valid.sum().clamp_min(1.0)
        else:
            jerk_loss = jerk_error.mean()
        loss = loss + float(jerk_weight) * jerk_loss

    metrics = {
        "loss": float(loss.detach().item()),
        "recon_loss": float(recon_loss.detach().item()),
        "kl_loss": float(kl_loss.detach().item()),
        "jerk_loss": float(jerk_loss.detach().item()),
    }
    return loss, metrics
